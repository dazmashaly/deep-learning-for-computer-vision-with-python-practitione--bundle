...
M = open(config.VAL_MAPP).read().strip().split("\n")
...
(R,G,B) = ([],[],[])
for(dType,paths,labels,outputpaths) in datasets:
    print("[INFO] building {}...".format(outputpaths))
    writer = hdf5datasetwriter.HDF5DatasetWriter((len(paths),64,64,3),outputpaths)
